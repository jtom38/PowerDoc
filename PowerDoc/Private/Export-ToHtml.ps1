...
.Example
Export-ToHtlm -Class -ClassName "ExampleClass" -BaseClasses $bc -Constructors $c -Properties $p -Methods $m -ClassHelp Get-ClassHelp -PathClassFile ".\PowerDoc\Classes\ExampleClass"
Export-ToHtml -Function -FunctionName "Export-ToHtml" -HelpDocs Get-Help 
            ...
            if ( $ClassHelp -ne ""){
                ...
                foreach( $k in $ClassHelp.GetEnumerator() ){
                    ...
                    foreach ($item in $k.Value){
                        Add-Content -Path $export -Value "$item\r"
                    }
                    Add-Content -Path $export -Value '<br>'
                }
            }
